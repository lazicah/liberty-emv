...
import com.libertypay.nexgoemvsdk.LibertyNexGoSDK
import com.libertypay.nexgoemvsdk.domain.models.AccountType
import com.libertypay.nexgoemvsdk.domain.models.RetrievalReferenceNumber
import com.libertypay.nexgoemvsdk.domain.models.TransactionAmount
...
class NexgoEmvService(private val context: Context) : LibertyEmv.LibertyEmvApi,
    ...
    override fun initialise(environment: LibertyEmv.Environment, result: LibertyEmv.Result<LibertyEmv.TransactionDataResponse>) {
        Timber.tag(ContentValues.TAG).d("initialize: sdk initializing")
        try {
            LibertyNexGoSDK.initialize(activityBinding!!.activity)
            ...
            Timber.tag(ContentValues.TAG).d("initialize error: %s", e.message)
        }
    ...
    override fun purchase(amount: String, accountType: LibertyEmv.AccountType, rrn: String, result: LibertyEmv.Result<LibertyEmv.TransactionDataResponse>) {
        ...
        scope.launch {
            ...
            val accountTypeEnum =
                    Constants.accountTypeMapNexgo[accountType] ?: AccountType.DEFAULT_UNSPECIFIED

            if (isSdkInitialised) {
                activityBinding?.activity?.let {
                    LibertyNexGoSDK.startPurchaseActivity(
                            activity = it,
                ...
                Timber.tag(ContentValues.TAG).d("Have you called [initialise]?")
            }
        }
    ...
    override fun performKeyExchange(result: LibertyEmv.Result<LibertyEmv.TransactionDataResponse>) {
        ...
        scope.launch {
            LibertyNexGoSDK.initialize(activityBinding!!.activity)
            if(isSdkInitialised) {
                val keyExchangeSuccess: Boolean = LibertyNexGoSDK.doKeyExchange()
    ...
    override fun print(bitmap: ByteArray, result: LibertyEmv.Result<LibertyEmv.TransactionDataResponse>) {
        ...
        scope.launch {
            try {
                ...
                LibertyNexGoSDK.printReceipt(value)
                ...
                Timber.tag(ContentValues.TAG).e("Printing Error: ${e.message}")
                val printResponse = LibertyEmv.TransactionDataResponse().apply {
    ...
    override fun enquireBalance(isOfflineTransaction: Boolean, accountType: LibertyEmv.AccountType, rrn: String, result: LibertyEmv.Result<LibertyEmv.TransactionDataResponse>) {
        ...
        scope.launch {
            ...
            if (isSdkInitialised) {
                activityBinding?.activity?.let {
                    LibertyNexGoSDK.startBalanceEnquiryDialogActivity(
                            activity = it,
                            terminalId = "",
                            accountType = accountTypeEnum,
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?): Boolean {
        Timber.tag(ContentValues.TAG).d("onActivityResult: %s", resultCode)
        Timber.tag(ContentValues.TAG).d("onActivityResult:changed")
